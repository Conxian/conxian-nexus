use conxian_nexus::executor::lightning::{
    LightningFailureType, LightningPaymentStatus, LightningResilienceAdapter,
};
...
fn test_lightning_resilience_logic() {
    ...
    assert_eq!(
        adapter.categorize_failure("permanent: no_route"),
        LightningFailureType::Permanent
    );
    assert_eq!(
        adapter.categorize_failure("transient: temporary_node_failure"),
        LightningFailureType::Transient
    );
    assert_eq!(
        adapter.categorize_failure("indeterminate: mpp_timeout"),
