...
#[tokio::test]
async fn test_bitvm_service_status() {
    let status = conxian_nexus::api::services::get_all_services_status();
    let bitvm = status.services.iter().find(|s| s.service_name == "BitVM");
    assert!(bitvm.is_some());
    assert_eq!(bitvm.unwrap().status, "Active");
}
