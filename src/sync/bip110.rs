...
mod tests {
    ...
    #[test]
    fn definite_violation_wins_over_unavailable_incomplete_or_unsupported_metadata() {
        for metadata in [
            ObservedSizeMetadata::new(
                ObservationAvailability::Unavailable,
                ObservationCoverage::Incomplete,
                vec![ObservedSizeItem::Pushdata {
                    size: MAX_PUSHDATA_SIZE + 1,
                }],
            ),
            ObservedSizeMetadata::incomplete(vec![
