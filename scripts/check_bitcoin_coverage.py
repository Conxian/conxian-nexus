...
SCOPED_LINE_RANGES: dict[str, list[tuple[int, int]] | None] = {
    ...
    "src/api/rest.rs": [
        (138, 252),  # app_router through get_rgb_contract
        (365, 387),  # aggregate BIP-110-only metrics response + handler
        (403, 442),  # get_mmr_proof handler
    ],
    "src/executor/rgb.rs": [
        (38, 45),   # rollout mode display
        (104, 148), # lookup validation + all rollout mode branches
    ],
}
