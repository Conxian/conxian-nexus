...
security. BIP-110 is a proposed consensus soft fork whose BIP status is
`Complete`; it is not `Deployed`, and it is not a light-client protocol.
...
## Likely sync-performance effects

If BIP-110 were activated and broadly enforced, its data-size ceilings could
reduce the worst-case bytes carried by the specific script and witness fields
covered by the proposal. A node that downloads affected blocks or transactions
could therefore see lower bandwidth, parsing work, temporary allocation, and
storage pressure for those fields. Those are directional expectations, not
measured Nexus results: Phase 1 has no Bitcoin backend, downloads no blocks,
and adds no benchmark data.

The proposal does not make a client light by itself. Header synchronization,
proof-of-work checking, transaction discovery, Merkle inclusion, reorg
handling, and any required UTXO or script validation still determine the sync
and security costs of a future Bitcoin observation path. Compact filters may
reduce candidate transaction downloads, but false positives still require
fetching and verifying the matching data. Phase 2 must measure the complete
pipeline rather than attribute an unmeasured speedup to these size limits.

## State-verification benefits and boundaries

Typed size observations provide a deterministic, auditable signal for the five
modeled proposal boundaries. Explicit availability and coverage prevent absent
metadata from being interpreted as success, while definite over-limit values
remain reportable even when other metadata is incomplete. Aggregate metrics can
then show whether observation coverage exists and which fixed rule classes are
being exceeded without exposing transaction-level identifiers.

This improves the quality of an observation signal; it does not prove state.
The assessor does not establish that a transaction is included in a block,
that the block belongs to the most-work valid chain, that an input spends the
claimed UTXO, that scripts succeed, or that activation and grandfathering make
a proposed rule applicable. Those claims require independently verified
headers and proof-of-work, Merkle inclusion, reorg-aware chain selection, and a
consensus-capable transaction/script boundary.
...
## Phase 2 light-verification and benchmark plan
...
| Reorganizations | Competing branches at depths 1, 2, 6, and 100, including a higher-work shorter branch | Select the valid most-work chain, roll back orphaned observations exactly once, and replay the winning branch without double-counting metrics |
...
| Activation and grandfathering | Pre-activation UTXOs spent after activation, post-activation UTXOs, activation-boundary blocks, expiry-boundary blocks, and reorgs across each boundary | Apply proposal rules only when deployment state and UTXO creation height make them applicable; never infer applicability from size metadata alone |
...
1. No invalid header, proof-of-work, Merkle proof, filter workflow, reorg, or
